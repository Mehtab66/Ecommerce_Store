  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/50 ${open ? '' : 'hidden'}`}>
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 relative">
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-2 right-2 text-gray-500 text-xl font-bold">×</button>
        ...
        <div className="grid grid-cols-2 gap-2 mt-6 bg-gray-100 rounded-xl overflow-hidden">
          <button
            ...
            className={`py-2 text-sm font-medium ${tab === "login" ? "bg-white shadow" : "text-gray-500"}`}
          >
            Login
          </button>
          <button
            onClick={() => setTab("signup")}
            className={`py-2 text-sm font-medium ${tab === "signup" ? "bg-white shadow" : "text-gray-500"}`}
          >
            Sign Up
          </button>
        </div>
        ...
        >
          <Globe size={18} /> {tab === "login" ? "Continue with Google" : "Sign Up with Google"}
        </button>
      </div>
    </div>
  );
